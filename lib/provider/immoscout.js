...
async function pushDetails(listing) {
  ...
  // The available-from date. Normal listings carry it as a labelled "Bezugsfrei ab:" attribute;
  // tenant-network / "available soon" listings usually carry it only in the free text
  // ("ab dem 01.10.2026 bezugsfrei", "Nachmieter:in ab dem 15.11.2026"). extractMoveInDate tries
  // the structured attribute first, then the free text, so both pools are covered by one helper.
  // Left undefined (not null) when nothing parseable is found, so the composer's fallback reads
  // the description instead - the AI path always reads the description anyway and is better at
  // fuzzy dates ("Ende September") than a strict parser.
  const moveInDate = extractMoveInDate(detailBody);
  if (moveInDate) {
...
/**
 * Find the listing's available-from (move-in) date in the parsed `/expose/{id}` response and
 * return it as an ISO string (YYYY-MM-DD), or null.
 *
 * Normal immoscout listings carry the date as a labelled attribute ("Bezugsfrei ab:",
 * "verfügbar ab:") in an ATTRIBUTE_LIST section. Tenant-network and "available soon" listings
 * usually omit that structured attribute and state the date only in the free text (TEXT_AREA),
 * phrased as a current tenant would: "ab dem 01.10.2026 bezugsfrei",
 * "Ich suche eine:n Nachmieter:in ab dem 15.11.2026". This helper checks the structured attribute
 * first (the precise source) and then scans the free text with patterns that cover both pools, so
 * the tenant-network provider reuses it unchanged - it inherits fetchDetails via its config.
 *
 * Returns null for "sofort" / "bezugsfrei" without a date, matching the composer's fallback which
 * treats a missing date as flexible.
 *
 * @param {any} detailBody Parsed `/expose/{id}` response.
 * @returns {string|null}
 */
export function extractMoveInDate(detailBody) {
  const sections = detailBody?.sections || [];

  // 1) structured attribute - any labelled "Bezugsfrei ab" / "verfügbar ab" / "bezug ab" / "einzug ab",
  //    across both TOP_ATTRIBUTES and ATTRIBUTE_LIST sections (the date sits in ATTRIBUTE_LIST).
  const labelRe = /^(bezugsfrei\s*ab|verfügbar\s*ab|bezug\s*ab|einzug\s*ab|einzugstermin|verfügbarkeitsdatum)/i;
  const attr = sections.flatMap((s) => s.attributes || []).find((a) => a?.label && labelRe.test(a.label));
  if (attr?.text) {
    const iso = parseDate(attr.text);
    if (iso) return iso;
    // "sofort" / "bezugsfrei" with no parseable date => available now => no structured date.
    if (/ab\s+sofort|sofort|^bezugsfrei$/i.test(attr.text.trim())) return null;
  }

  // 2) free text - the tenant-network phrasing. Ordered so the most specific cue wins.
  const freeText = sections
    .filter((s) => s.type === 'TEXT_AREA')
    .map((s) => s.text || '')
    .join('\n');
  if (/(ab\s+sofort|\bsofort\b)/i.test(freeText) && !/\d/.test(freeText)) return null;
  const cues = [
    /(?:verfügbar\s+ab|bezug\s*ab|bezugsfrei\s+ab|einzug\s*ab)\s*:?(?:\s+dem\s*|\s+)?:?\s*([^,\n;]+)/i,
    /nachmieter[^.\n]*?\bab\s+dem?\s+([^,\n;]+)/i,
    /ab\s+de[ms]\s+([^,\n;]+?)\s+bezugsfrei/i,
  ];
  for (const cue of cues) {
    const m = cue.exec(freeText);
    if (m) {
