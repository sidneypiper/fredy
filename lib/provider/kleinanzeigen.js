...
function extractDetailFromHtml(html) {
  ...
  // Seller display name + whether it is a commercial seller. Private ads carry a person's name in
  // the profile box; commercial ads carry the shop/agency name and a "Gewerblicher Nutzer" badge.
  // Prefixed onto the description as an "Agent:" line (the same convention immoscout uses) so the
  // message composer can address the seller by name and fall back to "Damen und Herren" for a
  // business.
  const profileBox = $('#viewad-profile-box').first();
  let detailContactName = cleanText(profileBox.find('span.userprofile-vip.text-force-linebreak').first().text());
  if (!detailContactName) {
    const placeholder = $('#viewad-contact-message').attr('placeholder') || '';
    const m = /Schreibe eine freundliche Nachricht an (.+?) und bekomm/.exec(placeholder);
    if (m) detailContactName = cleanText(m[1]);
  }
  const detailIsCommercial = /Gewerblich/i.test(profileBox.text());
...
async function enrichListingFromDetails(listing, browser) {
  ...
  try {
    ...
    const { detailAddress, detailDescription, detailSize, detailRooms, detailContactName, detailIsCommercial } =
      extractDetailFromHtml(html);

    let description = detailDescription || listing.description;
    if (detailContactName) {
      const tag = detailIsCommercial ? `${detailContactName} (gewerblich)` : detailContactName;
      description = description ? `Agent: ${tag}\n\n${description}` : `Agent: ${tag}`;
    }
