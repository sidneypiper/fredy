...
async function fetchDetails(listing, browser) {
  try {
    ...
    $('script[type="application/ld+json"]').each((_, el) => {
      ...
      try {
        // immobilien.de's RealEstateListing block is not valid JSON: the description string carries
        // raw newlines / control characters that JSON.parse rejects (the price extractor notes the
        // same and falls back to a text scan). Replace control chars with spaces so the block
        // parses. This is safe because JSON never legitimately contains raw control characters
        // outside escaped string values, so only the offending in-string bytes are touched.
        const raw = $(el)
          .text()
