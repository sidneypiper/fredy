...
import {
  ...
  updateListingPersonalizedMessage,
...
import { distancesToAddresses, distanceToPolygonMeters } from './services/listings/distanceCalculator.js';
import { updateTravelTimesForListings } from './services/listings/travelTimeSweeper.js';
import { roundCoord } from './services/transit/transitCache.js';
import { getSettings, getUserSettings, getAddresses } from './services/storage/settingsStorage.js';
import { createComposerFromSettings } from './services/ai/messageComposer.js';
...
/**
 * How far outside the drawn area a *coarse* geocode (postcode/city centroid) may land and still be
 * kept. A postcode centroid is not a front door, so a flat whose centroid falls just outside the
 * polygon can still sit inside it; only a centroid farther than this from every edge is dropped.
 * German postcodes range from ~1-3 km (city) to ~10-20 km (rural), so 5 km keeps anything
 * plausibly in range and only drops clearly-out-of-area ones.
 */
const DEFAULT_COARSE_AREA_BUFFER_METERS = 5000;
 ...
 * 9) Calculate distances to the user's addresses
 * 10) Filter out entries similar to already seen ones
 * 11) Filter out entries that do not match the job's spatialFilter
 * 12) Calculate travel times to the user's addresses
 * 13) Filter out entries that do not match the job's travelTimeCondition
 * 14) Optionally enrich new listings via provider.fetchDetails - after every
 *     filter, so no detail page is ever fetched for a listing the filters would
 *     reject anyway
 * 15) Optionally re-apply the provider blacklist using the (now enriched)
...
class FredyPipelineExecutioner {
  ...
  execute() {
    return (
      Promise.resolve(urlModifier(this._providerConfig.url, this._providerConfig.sortByDateParam))
        ...
        // Geocoding moved ahead of the detail fetch: it is what the travel time filter and the
        // area filter read, and no provider's fetchDetails touches the address, so nothing is lost
        // by geocoding the search-list address first.
        ...
        .then(this._filterByTravelTime.bind(this))
        // Enrichment runs after every filter - specs, geocode, similarity, area and travel time -
        // so no detail page is ever fetched for a listing the filters would reject anyway, which
        // is what keeps the provider API calls small.
        .then(this._fetchDetails.bind(this))
        .then(this._filterAfterDetails.bind(this))
        .then(this._generatePersonalizedMessages.bind(this))
  ...
  async _fetchDetails(newListings) {
    ...
    for (const listing of listingsToEnrich) {
      ...
      // Enrichment runs after the listings were stored, so the enriched fields have to be written
      // back to the row the pipeline already inserted - otherwise the detail page would only ever
      // reach the notification and never the database.
      updateListingDetails(detailed.id, {
    ...
    for (const listing of newListings) {
      if (listing.address) {
        ...
        if (coords && coords.lat !== -1 && coords.lng !== -1) {
          ...
          listing.geocodePrecision = coords.precision ?? 'exact';
        }
      }
    }
    return newListings;
   ...
   *
   * A *coarse* geocode (postcode or city centroid) is not a front door, so a centroid that lands
   * just outside the polygon can still belong to a flat inside it. Such listings are kept unless
   * the centroid is farther than the configured buffer from every polygon edge - far enough that
   * even the postcode's spread cannot reach the area. Exact coordinates are filtered strictly, as
   * before.
   *
   * @param {ParsedListing[]} newListings New listings to filter by area.
   * @returns {Promise<ParsedListing[]>} Resolves with listings that are within the area (or not
   * filtered if no area is set).
   */
  async _filterByArea(newListings) {
    ...
    const coarseBufferMeters = positiveInt(settings?.coarseAreaBufferMeters, DEFAULT_COARSE_AREA_BUFFER_METERS);
    ...
    const keptListings = newListings.filter((listing) => {
      ...
      // Outside the polygon. A coarse centroid is only an approximation of where the flat is, so
      // it is kept when it is still within the buffer of the area - the postcode could overlap it.
      // An exact coordinate, or a coarse one that is clearly out of range, is dropped.
      if (listing.geocodePrecision === 'coarse') {
        const nearest = Math.min(
          ...polygonFeatures.map((feature) => distanceToPolygonMeters(listing.latitude, listing.longitude, feature)),
        );
        if (Number.isFinite(nearest) && nearest <= coarseBufferMeters) {
          return true;
        }
      }
  ...
  /**
   * Filter listings by the job's travel time condition.
   *
   * The condition is a filter like the area filter: a listing that is not within the configured
   * commute of any selected place is soft-deleted here and never reaches enrichment or a
   * notification. It runs after the travel times were computed by {@link _calculateTravelTimes},
   * so it costs no requests of its own.
   *
   * A listing with no coordinates at all cannot be measured and is rejected. A place the
   * condition references by coordinates but that no longer exists among the stored travel times
   * (deleted from the settings, or never reachable) simply does not count. If the whole run
   * produced no travel times at all - the routing service being paused or down - the filter is
   * skipped entirely: an outage must not wipe a job's results.
   *
   * @param {ParsedListing[]} newListings New listings to filter.
   * @returns {ParsedListing[]} Listings within the configured commute of at least one place.
   * @throws {NoNewListingsWarning} When every listing is filtered out.
   */
  _filterByTravelTime(newListings) {
    const condition = getJob(this._jobKey)?.travelTimeCondition;
    const places = condition?.places;
    // No filter configured means no filtering: the job keeps everything, exactly as before.
    ...
    const byListing = getTravelTimesForListings(newListings.map((listing) => listing.id));
    const matchesPlace = (row, place) => {
      const lat = Number(place.lat);
      const lng = Number(place.lng);
      return (
        Number.isFinite(lat) &&
        Number.isFinite(lng) &&
        row.origin_lat === roundCoord(lat) &&
        row.origin_lng === roundCoord(lng)
      );
    };

    // Fail open when the condition cannot be evaluated at all: no stored travel time for any of
    // the run's listings matches any selected place. That is the routing service being down, or
    // the selected places having been deleted - either way it is infrastructure, not the listing,
    // and wiping the run over it would be wrong.
    const evaluable = [...byListing.values()].some((rows) =>
      rows.some((row) => places.some((place) => matchesPlace(row, place))),
    );
    if (!evaluable) {
      logger.warn(
        `Travel time condition for job '${this._jobKey}' could not be evaluated (no stored travel times for the selected places). Keeping all listings.`,
      );
      return newListings;
    }
    ...
    const rejected = [];
    const keptListings = newListings.filter((listing) => {
      // No geocode at all means the listing cannot be measured against any place, so it cannot
      // pass. With the fallback geocoder this is the rare edge case.
      if (
        ...
        rejected.push(listing.id);
        return false;
      }

      const precision = listing.geocodePrecision === 'coarse' ? 'coarse' : 'exact';
      const rows = byListing.get(listing.id) ?? [];
      const passes = places.some((place) => {
        const row = rows.find((candidate) => matchesPlace(candidate, place));
        if (!row) {
          return false;
        }
        // The row knows which mode it was measured in; its minutes live in that mode's column.
        const mode = row.estimate_mode ?? 'transit';
        const minutes = row[`${mode}_minutes`];
        if (minutes == null) {
          return false;
        }
        const threshold = precision === 'coarse' ? place.coarseMaxMinutes : place.exactMaxMinutes;
        return Number.isFinite(Number(threshold)) && minutes <= Number(threshold);
      });
      if (!passes) {
        rejected.push(listing.id);
      }
    ...
    if (rejected.length > 0) {
      logger.debug(`Travel time filter removed ${rejected.length} listing(s) (Provider: '${this._providerId}')`);
      deleteListingsById(rejected);
    }
  ...
  _filterAfterDetails(listings) {
    ...
    // Enrichment now runs after the listings were stored, so a listing rejected here has already
    // been persisted and has to be removed again - it must not sit in the database as if the job
    // had kept it.
    const removedIds = [];
    const kept = listings.filter((listing) => {
      const keep = this._providerConfig.filter(listing);
      if (!keep) {
  ...
  /**
   * Generate the AI-personalized message for every listing that survived all filters.
   *
   * Runs after enrichment and the detail re-filter, so the model only ever sees listings the
   * user will actually be notified about - the same cost discipline as the detail fetch. The
   * composer is created lazily from the global AI settings and cached on the instance, so a
   * routing outage trips the shared circuit breaker once instead of on every listing.
   *
   * Fail-open: when the job has the feature disabled, the AI is not configured, or the model
   * is unreachable, the listings pass through unchanged and the pipeline continues. The
   * composer itself falls back to a template-only fill when the model fails, so a configured
   * job still gets a message with the placeholders filled deterministically.
   *
   * The base text is chosen per provider: a `perProvider` override for this run's provider id
   * wins over the job-wide `baseText`, so a job can use a formal template for landlord portals
   * and a casual one for tenant-network listings. A provider with neither set gets no message.
   *
   * @param {ParsedListing[]} newListings Listings to generate messages for.
   * @returns {Promise<ParsedListing[]>} The same listings, with `personalizedMessage` set where generated.
   */
  async _generatePersonalizedMessages(newListings) {
    const job = getJob(this._jobKey);
    const condition = job?.personalizedMessage;
    if (!condition?.enabled) {
      return newListings;
    }
    // A per-provider base text overrides the job-wide default for this run's provider; providers
    // without an entry fall back to the default. A provider with neither set gets no message.
    const baseText = condition.perProvider?.[this._providerId] ?? condition.baseText;
    if (!baseText?.trim()) {
      return newListings;
    }
    if (!this._composer) {
      const settings = getUserSettings(job?.userId);
      this._composer = createComposerFromSettings(settings);
      if (!this._composer) {
        logger.debug(`Personalized messages enabled for job '${this._jobKey}' but the AI is not configured - skipping`);
        return newListings;
      }
    }
    for (const listing of newListings) {
      const result = await this._composer.compose(listing, baseText);
      if (result) {
        listing.personalizedMessage = result.body;
        // The message is generated after the listing was stored, so it has to be written back to
        // the row the pipeline already inserted - otherwise it would only ever reach the
        // notification and never the database.
        updateListingPersonalizedMessage(listing.id, result.body);
      }
    }
    return newListings;
  }
