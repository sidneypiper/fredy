...
class FredyPipelineExecutioner {
  ...
  async _generatePersonalizedMessages(newListings) {
    const condition = getJob(this._jobKey)?.personalizedMessage;
    if (!condition?.enabled || !condition.baseText?.trim()) {
      return newListings;
    }
    if (!this._composer) {
      const settings = await getSettings();
    ...
    for (const listing of newListings) {
      const result = await this._composer.compose(listing, condition.baseText);
      if (result) {
