...
export default async function userSettingsPlugin(fastify) {
  fastify.get('/', async (request) => {
    ...
    // The AI key is a secret like any other: it must never travel to a browser. The UI only
    // needs to know whether one is stored, to decide between "enter a key" and "keep the
    // stored one".
    const aiApiKeySet = Boolean(settings.ai_api_key);
    delete settings.ai_api_key;
    settings.ai_api_key_set = aiApiKeySet;
    return settings;
  });
  ...
  fastify.get('/geocode', async (request, reply) => {
    ...
    try {
      ...
      // Precision is a listing concept; a home-address search returns only the coordinate the UI
      // asked for.
  ...
  fastify.post('/home-address', async (request, reply) => {
    ...
    try {
      ...
      for (const { label, address, departure, mode } of entries) {
        const result = (await geocodeAddress(address)) || { lat: -1, lng: -1 };
        // Only the coordinate is stored on a home address; precision is a listing concept and would
        // only collect here as an unused field.
  ...
  // The AI provider is configured in the settings area like the other per-user preferences
  // (provider details, addresses): each user picks their own model and key. The key is stored in
  // the settings table like every other secret and stripped from the GET response above.
  fastify.post('/ai-settings', async (request, reply) => {
    const userId = request.session.currentUser;
    const { ai_provider, ai_model, ai_api_key } = request.body;
    ...
    if (ai_provider != null && ai_provider !== 'ollama') {
      return reply.code(400).send({ error: 'Unsupported AI provider.' });
    }
    if (ai_model != null && typeof ai_model !== 'string') {
      return reply.code(400).send({ error: 'ai_model must be a string.' });
    }
    if (ai_api_key != null && typeof ai_api_key !== 'string') {
      return reply.code(400).send({ error: 'ai_api_key must be a string.' });
    }

    try {
      // Only fields the client actually sent are applied: an omitted ai_api_key means "keep the
      // stored one", an explicit null means "delete it", a string means "replace it".
      const updates = {};
