...
import { PERSONALIZED_MESSAGE_LABEL, personalizedMessageSection } from '../personalizedMessage.js';
 ...
 * @param {Object} o - Listing object
 * @param {string} [baseUrl]
 * @param {boolean} [includePersonalizedMessage=true] - When false, the landlord message block
 *   is omitted (used when it is sent as its own message instead).
 * @returns {string}
 */
function buildHtmlBody(jobName, serviceName, o, baseUrl, includePersonalizedMessage = true) {
  ...
  const message =
    includePersonalizedMessage && o.personalizedMessage
      ? `\n\n<b>${escapeHtml(PERSONALIZED_MESSAGE_LABEL)}:</b>\n${escapeHtml(o.personalizedMessage)}`
      : '';
  return (
    `<i>${escapeHtml(jobName)}</i> (${escapeHtml(serviceName)})\n` +
    `<a href='${escapeHtml(o.link || '')}'><b>${escapeHtml(title)}</b></a>\n` +
    `${escapeHtml(meta)}${fredyLink}${message}`
  );
}
 ...
 * @returns {string}
 */
function buildPlainCaption(jobName, serviceName, o, baseUrl, includePersonalizedMessage = true) {
  const title = shorten((o.title || '').replace(/\*/g, ''), 90);
  const meta = [o.address, o.price, o.size, o.commute].filter(Boolean).join(' | ');
  const fredyLine = baseUrl && o.id ? `\nOpen in Fredy: ${baseUrl}/#/listings/listing/${o.id}` : '';
  const pm = includePersonalizedMessage ? personalizedMessageSection(o) : '';
  return `${jobName} (${serviceName})\n${title}\n${meta}\n\n${o.link || ''}${fredyLine}${pm}`.slice(0, 4096);
}
 ...
 * @returns {string}
 */
function buildPlainText(jobName, serviceName, o, baseUrl, includePersonalizedMessage = true) {
  const title = shorten((o.title || '').replace(/\*/g, ''), 90);
  const meta = [o.address, o.price, o.size, o.commute].filter(Boolean).join(' | ');
  const fredyLine = baseUrl && o.id ? `\nOpen in Fredy: ${baseUrl}/#/listings/listing/${o.id}` : '';
  const pm = includePersonalizedMessage ? personalizedMessageSection(o) : '';
 ...
 * @param {boolean} [opts.sendLandlordMessageSeparately=false]
...
async function sendListingToChat(
  ...
  listing,
  chatId,
  { jobName, serviceName, baseUrl, plainText, message_thread_id, sendLandlordMessageSeparately = false },
) {
  const img = normalizeImageUrl(listing.image);
  const includeMsg = !sendLandlordMessageSeparately;

  const textPayload = {
    chat_id: chatId,
    text: plainText
      ? buildPlainText(jobName, serviceName, listing, baseUrl, includeMsg)
      : buildHtmlBody(jobName, serviceName, listing, baseUrl, includeMsg),
  ...
  if (!img) {
    await throttledCall('sendMessage', textPayload).catch((e) => {
      logger.error(`Error sending message to Telegram: ${e.message}`);
    });
  } else {
    const caption = plainText
      ? buildPlainCaption(jobName, serviceName, listing, baseUrl, includeMsg)
      : buildHtmlBody(jobName, serviceName, listing, baseUrl, includeMsg).slice(0, 1024);
  ...
  // Optionally send the "Message to the landlord" as its own plain-text message so it can be
  // copied on its own (long-press → Copy). No header is added so the copied text is exactly the
  // message to send to the landlord. Sent after the listing so it lands right below it.
  if (sendLandlordMessageSeparately && listing.personalizedMessage) {
    const landlordPayload = {
      chat_id: chatId,
      text: listing.personalizedMessage,
      disable_web_page_preview: true,
      ...(message_thread_id ? { message_thread_id } : {}),
    };
    await throttledCall('sendMessage', landlordPayload).catch((e) => {
      logger.error(`Error sending landlord message to Telegram: ${e.message}`);
    });
  }
}
...
export const send = ({ serviceName, newListings = [], notificationConfig, jobKey, baseUrl }) => {
  ...
  const { token, chatId, messageThreadId, plainText, sendLandlordMessageSeparately } = adapterCfg.fields;
  if (!token || !chatId) {
  ...
  // Listings are sent sequentially within each chat (await each listing fully before the next).
  // This keeps a listing's optional landlord message directly below that listing instead of
  // after every listing's main message. The per-chat throttle already serialises the actual API
  // calls to 1/sec, so this reorders without losing throughput for a single chat. Chats remain
  // concurrent with each other.
  const allPromises = chatIds.map(async (id) => {
    const caller = makeTelegramCaller(token, jobName);
    const throttledCall = getThrottled(id, caller);
    const opts = { jobName, serviceName, baseUrl, plainText, message_thread_id, sendLandlordMessageSeparately };
    for (const listing of newListings) {
      await sendListingToChat(throttledCall, listing, id, opts);
    }
  });
...
export const sendPriceChange = ({ serviceName, priceChanges = [], notificationConfig, jobKey, baseUrl }) => {
  ...
  const allPromises = chatIds.map(async (id) => {
    ...
    for (const change of priceChanges) {
      await sendListingToChat(throttledCall, toPriceChangeListing(change), id, opts);
    }
  });
...
export const config = {
  ...
  fields: {
    ...
    sendLandlordMessageSeparately: {
      type: 'boolean',
      optional: true,
      label: 'Send landlord message separately',
      description:
        'Send the "Message to the landlord" as its own plain-text message so you can copy it on its own. When on, it is removed from the listing message and sent right below it.',
    },
  },
};
