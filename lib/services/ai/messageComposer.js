 ...
 * UI hint stay in one place: {{GREETING}}, {{AD_SENTENCE}}, {{MOVE_IN_DATE}}, {{AGREEMENTS}}.
 *
 * The ad sentence highlights what sets the apartment apart, in the "Besonders der ..." style:
 * one concrete, listing-grounded sentence (old-building charm, the balcony, the quiet courtyard)
 * that shows the applicant read the text - same words allowed, never a verbatim copy of a
 * listing sentence. {{AGREEMENTS}} is where the user wants the practical details (furniture
 * take-over, kitchen purchase) acknowledged, with the price the listing states.
 */
...
4. {{AD_SENTENCE}}: Replace with exactly ONE sentence that highlights what is special about THIS apartment - what sets it apart from others. Base it only on the listing data: the enriched description, the title, the location. Examples: the old-building charm and high ceilings, the large balcony, the quiet courtyard location, the new fitted kitchen, the garden. The sentence must show the applicant read the listing carefully and is specifically interested in THIS apartment. You MAY use the same words the listing uses, but the sentence must not be a verbatim copy of any listing sentence. Do NOT invent details; when unsure, phrase cautiously ("as far as I can see from the listing"). NEVER output the placeholder itself.
...
6. {{MOVE_IN_DATE}}: Replace with the listing's available-from date ("Verfügbar ab" / "available from" / "move-in date" in the enriched description), formatted DD.MM.YYYY (e.g. 01.11.2026), never ISO. If the listing is available immediately or the date is unknown, write something like "flexible move-in" in the base text's language.
7. {{AGREEMENTS}}: Replace with a short statement covering every agreement the listing mentions - furniture that can be taken over (Möbelübernahme), a kitchen that must be bought, and so on. For each agreement, say it is no problem for the applicant and that they are willing to pay the price the listing states (e.g. "the take-over of the furniture is no problem for me, and I am willing to pay the stated 500 €"). If the listing states no price, just say it is no problem. If the listing mentions no agreements, replace the placeholder with an empty string.
8. If the listing data is missing or empty: omit {{AD_SENTENCE}} entirely, invent nothing.
9. Length: at most {max_chars} characters, no line longer than ~90 characters. No Markdown, no bullet points.
10. Do NOT mention the rent price or the number of rooms.
11. Output a finished, directly sendable message.
