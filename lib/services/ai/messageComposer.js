...
PLACEHOLDERS:
{{GREETING}} - The salutation that fits the base text. For a formal base text (e.g. "Sehr geehrte {{GREETING}}"), fill with "Frau <lastname>" or "Herr <lastname>" from the listing's "Agent:" line (if the Agent line already says "Herr" or "Frau", use that; otherwise choose Frau/Herr from the first name); if the agent is unknown or generic ("Unbekannt"), fill with "Damen und Herren". If the Agent line is a business or agency rather than a person (it carries "(gewerblich)" or contains a company marker such as GmbH, AG, IMMOBILIEN, Hausverwaltung, Shop, Makler, or is clearly not a personal name), fill with "Damen und Herren" rather than forcing Frau/Herr. For an informal base text (e.g. "Hallo {{GREETING}}", a casual peer message to a current tenant / Nachvermietung), use the contact's first name when the Agent line gives a personal name, or "Frau/Herr <lastname>" when only a lastname is available. If the agent is unknown/generic ("Unbekannt" or no personal name at all), write the greeting as just "Hallo," - the word Hallo followed by a comma, with no name - rather than "Hallo Damen und Herren" (a formal unknown-agent salutation clashes with the Du body).
 ...
 *    throughout; example 6 shows the Du text with an UNKNOWN agent - the greeting is just "Hallo,"
 *    (no name) instead of "Hallo Damen und Herren". {{AD_SENTENCE}}/{{AGREEMENTS}} adapt to the Du
 *    register in both.
 */
...
// `parseDate` and the month tables live in ../utils/parse-date.js so the immoscout provider can
// extract a listing's available-from date from the expose without depending on the AI service.
// Re-exported here so the existing tests and callers that import it from the composer keep working.
export { parseDate };
...
export function findMoveInDate(description) {
  ...
  if (/(ab\s+sofort|\bsofort\b|immediately|available\s+now)/i.test(description)) return null;
  const patterns = [
...
export function parseAgentName(description) {
  ...
  // "Agent: Unbekannt" is immoscout's placeholder for an unnamed agent; treat it as no name so
  // the greeting falls back rather than addressing "Unbekannt".
  return name && /^unbekannt$/i.test(name) ? null : name;
}
...
/**
 * Whether a base text is the informal Du register rather than the formal Sie. The Du pronouns and
 * possessives (deine, dir, dich, ...) outweigh the formal Sie/Ihnen/Ihre ones. Only the
 * template-only fallback uses this - the AI reads the base text itself - to pick a greeting that
 * matches the register when the AI is unavailable.
 *
 * @param {string} text
 * @returns {boolean}
 */
export function isInformal(text) {
  const t = text || '';
  const du = (t.match(/\b(?:du|dir|dich|deine|deiner|deinen|euch|euer|eure)\b/gi) || []).length;
  const sie = (t.match(/\b(?:Sie|Ihnen|Ihre|Ihren|Ihr)\b/g) || []).length;
  return du > 0 && du >= sie;
}

/**
 * Whether an "Agent:" name is a business/agency rather than a person, so the greeting falls back
 * to "Damen und Herren" / "Hallo," instead of "Frau/Herr <agency>". Catches the (gewerblich) marker
 * the kleinanzeigen enrichment adds and common German real-estate company tokens.
 *
...
export function looksLikeBusiness(name) {
  ...
  return /(gewerblich|gmbh|\bAG\b|immobilien|hausverwaltung|\bshop\b|&\s*co|makler|verwaltung|von poll|engel & völkers|remax|century 21)/i.test(
    name,
  );
}

/**
 * Pick the template-only fallback greeting for the base text's register and the seller name: the
 * first name for an informal (Du) text when a person is named, "Hallo," when no name is known, and
 * the formal "Sehr geehrte Frau/Herr <name>" / "Damen und Herren" for a Sie text - the latter also
 * when the seller is a business, so an agency is never addressed as "Frau/Herr <agency>".
 *
 ...
 * @returns {string}
 */
function resolveGreeting(baseText, agentName, lang) {
  const isUnknown = !agentName || /^unbekannt$/i.test(agentName.trim());
  const personName = !isUnknown && !looksLikeBusiness(agentName) ? agentName : null;
  const first = (n) => n.trim().split(/\s+/)[0];
  if (lang !== 'de') {
    return personName ? `Mr./Ms. ${personName}` : 'Sir or Madam';
  }
  if (isInformal(baseText)) {
    return personName ? first(personName) : '';
  }
  return personName ? `Frau/Herr ${personName}` : 'Damen und Herren';
}
...
export function renderBaseText(baseText, { greeting, moveIn, adSentence = null, agreements = '' }) {
  ...
  // An empty greeting (unknown agent, informal "Hallo {{GREETING}}," base text) leaves "Hallo ,";
  // collapse the stray space so the line reads "Hallo,".
  if (!greeting) {
...
export class MessageComposer {
  ...
  fallback(listing, baseText) {
    ...
    const moveInIso = listing.moveInDate ?? findMoveInDate(listing.description);
    ...
    const body = renderBaseText(baseText, {
      greeting: resolveGreeting(baseText, agentName, lang),
      moveIn,
      adSentence,
