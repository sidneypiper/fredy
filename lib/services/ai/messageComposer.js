 ...
 * sentence - to show the applicant actually read the text without quoting it. Furniture and
 * kitchen details are always acknowledged, but as a closing sentence - they are practical
 * facts, not the hook.
 */
...
4. {{AD_SENTENCE}}: Replace with exactly ONE sentence that shows the applicant read the listing carefully. The psychological goal is to MIRROR the landlord's own style and reasoning: pick up the points the listing itself emphasizes and echo them back, as a reflection. If the listing stresses the quiet courtyard, say the quiet courtyard; if it stresses the old-building charm, say the old-building charm. You MAY use the same words and terms the listing uses - but the sentence must not be a verbatim copy of any sentence from the listing: rephrase the structure so it reads as the applicant's own reaction, not a quote. Do NOT draw your own conclusions or causal reasoning ("the living is very efficient because of the kitchen") - that reads as artificial. Do NOT invent details; when unsure, phrase cautiously ("as far as I can see from the listing"). NEVER output the placeholder itself.
...
7. FURNITURE AND KITCHEN: If the listing mentions furniture that can be taken over (Möbelübernahme) or specifics about the kitchen (e.g. fitted kitchen, kitchen must be bought), ALWAYS mention it in the message - it shows the applicant read the listing and is ready for the practical details. Put this at the END of the message, as a short closing sentence, NOT in {{AD_SENTENCE}}.
