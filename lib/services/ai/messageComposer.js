 ...
 * to forget a constraint buried after ten content rules. Five worked examples follow in the
...
PLACEHOLDERS:
{{GREETING}} - The salutation that fits the base text. For a formal base text (e.g. "Sehr geehrte {{GREETING}}"), fill with "Frau <lastname>" or "Herr <lastname>" from the listing's "Agent:" line (if the Agent line already says "Herr" or "Frau", use that; otherwise choose Frau/Herr from the first name); if the agent is unknown or generic ("Unbekannt"), fill with "Damen und Herren". For an informal base text (e.g. "Hallo {{GREETING}}", a casual peer message to a current tenant / Nachvermietung), use the contact's first name when the Agent line gives a personal name; otherwise fall back to "Frau/Herr <lastname>", or "Damen und Herren" if the agent is unknown.
...
Five worked examples follow in the conversation below. Match their style exactly.`;
...
/**
 * Five worked input/output pairs for the few-shot prefix. Each input is the real base text plus
 * a real listing; each output is the finished message the user approved. They cover the variations
 * a small model needs to see:
 *  - unknown vs. named agent (examples 1 and 4 vs. 2, 3 and 5),
 *  - an ad sentence when the listing has a distinctive detail vs. NO ad sentence when it is
 *    generic (examples 2-5 vs. 1) - the model must leave {{AD_SENTENCE}} empty rather than invent
 *    generic praise,
 *  - no agreements vs. a kitchen/furniture take-over (examples 1, 2, 4 and 5 vs. 3) - and, crucially,
 ...
 *  - a fixed date vs. a flexible move-in (examples 1, 3, 4 and 5 vs. 2),
 *  - a formal (Sie) base text to a landlord/agent vs. a casual (Du) base text to a current tenant
 *    / Nachvermietung (examples 1-4 vs. 5): example 5 uses the first name in the greeting and Du
 *    throughout, and {{AD_SENTENCE}}/{{AGREEMENTS}} adapt to the Du register.
 */
...
Einziehen kann ich ab dem {{MOVE_IN_DATE}}. Für ein kurzes Kennenlernen komme ich gerne kurzfristig vorbei. Am schnellsten erreichst du mich unter +4915112345678.
...
Einziehen kann ich ab dem 30.09.2026. Für ein kurzes Kennenlernen komme ich gerne kurzfristig vorbei. Am schnellsten erreichst du mich unter +4915112345678.
