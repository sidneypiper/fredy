 ...
 * to forget a constraint buried after ten content rules. Three worked examples follow in the
...
PLACEHOLDERS:
{{GREETING}} - A polite salutation in the base text's language. Use the contact name from the listing's "Agent:" line: "Sehr geehrte Frau <lastname>" or "Sehr geehrter Herr <lastname>" (if the Agent line already says "Herr" or "Frau", use that; otherwise choose Frau/Herr from the first name). If the agent is unknown or generic ("Unbekannt"), use "Sehr geehrte Damen und Herren".

{{AD_SENTENCE}} - Exactly ONE sentence about what makes THIS apartment special, grounded only in the listing's description, title and address. Show the applicant read the listing. You may reuse the listing's words, but never copy a whole sentence verbatim. Do not invent details. Never output the placeholder itself.
...
{{AGREEMENTS}} - A short sentence acknowledging every condition the listing names: a kitchen or furniture take-over (Möbelübernahme/Küche), an unrenovated handover (unrenoviert), and so on. For each, say it is no problem for the applicant and, if a price is stated, that they are willing to pay it. If the listing names no such condition, replace the placeholder with an empty string and remove the blank line it leaves.
...
Three worked examples follow in the conversation below. Match their style exactly.`;

/**
 * The applicant's base text used in the worked examples. It is the real template from the
 * running job, with the four placeholders; the examples show how each one is filled from a real
 * listing. The same text is sent for every example so the model sees that only the placeholders
 * change while the fixed text stays verbatim.
 */
const EXAMPLE_BASE_TEXT = `Sehr geehrte {{GREETING}},

Mein name ist Sidney Piper und ich bin 24 Jahre alt. Ich bin ein ruhiger Mieter, Nichtraucher und habe keine Haustiere. Als Softwareentwickler bei REWE digital verfüge ich über ein stabiles Netto-Einkommen von ca. 2.000 €.
...
Ich kann ab dem {{MOVE_IN_DATE}} einziehen und stehe für Besichtigungen kurzfristig zur Verfügung. Sie erreichen mich telefonisch am besten unter +4915735577455.
...
/**
 * Three worked input/output pairs for the few-shot prefix. Each input is the real base text plus
 * a real listing from the database; each output is the finished message the user approved. They
 * cover the variations a small model needs to see: unknown vs. named agent, no agreements vs.
 * a kitchen/furniture take-over, and a fixed date vs. a flexible move-in.
 */
...
Besonders gut gefällt mir die durchdachte Raumaufteilung dieser Wohnung in der Rolshover Straße, die auf kompakter Fläche ein funktionales Wohnen ermöglicht.
...
Ich kann ab dem 13.11.2026 einziehen und stehe für Besichtigungen kurzfristig zur Verfügung. Sie erreichen mich telefonisch am besten unter +4915735577455.
  ...
  },
  {
    input: {
      ...
      listing: {
        ...
        description:
          'Agent: Gia-Tri Lam\n\nBeschreibung\nDiese ansprechende 1,5-Zimmer-Wohnung bietet auf 44 Quadratmetern eine durchdachte Aufteilung, die sich ideal für einen unkomplizierten Alltag eignet. Die Räumlichkeiten sind hell gestaltet und bieten ausreichend Platz für eine individuelle Einrichtung.',
      },
    },
    output: {
      subject: 'Kompakt wohnen in Köln: 1,5-Zimmer-Wohnung in der Holweider Straße',
      body: `Sehr geehrte Frau Lam,

Mein name ist Sidney Piper und ich bin 24 Jahre alt. Ich bin ein ruhiger Mieter, Nichtraucher und habe keine Haustiere. Als Softwareentwickler bei REWE digital verfüge ich über ein stabiles Netto-Einkommen von ca. 2.000 €.

Besonders ansprechend finde ich die durchdachte Raumaufteilung dieser hellen Wohnung, die trotz der kompakten Größe Platz für eine individuelle Einrichtung lässt.

Um Ihnen als Vermieterin absolute finanzielle Sicherheit und Planbarkeit zu garantieren, habe ich zudem eine vollumfängliche, unterschriebene Elternbürgschaft vorbereitet. Diese bringe ich gerne zur Besichtigung mit oder schicke ich Ihnen vorab als PDF.

Meine Bonität und Einkommenssituation können Sie sofort einsehen: Meine geprüfte Bewerbermappe (inkl. aktueller Schufa, Gehaltsnachweisen und verifizierten Mietzahlungen) habe ich in dieser Anfrage bereits vollständig für Sie freigeschaltet.

Ich kann flexibel einziehen und stehe für Besichtigungen kurzfristig zur Verfügung. Sie erreichen mich telefonisch am besten unter +4915735577455.
...
Ich kann ab dem 10.10.2026 einziehen und stehe für Besichtigungen kurzfristig zur Verfügung. Sie erreichen mich telefonisch am besten unter +4915735577455.
...
export function findAgreements(description) {
  if (!description) return null;
  const hasAgreement =
    /(möbelübernahme|furniture|einbauküche|fitted kitchen|küche|kitchen|übernahme|take[- ]?over)/i.test(description);
  if (!hasAgreement) return null;
  // A price near the agreement keyword, not the first price in the text (which is usually the rent).
  const priceMatch =
    /(?:möbelübernahme|furniture|einbauküche|fitted kitchen|küche|kitchen|übernahme|take[- ]?over)[^.]{0,120}?(\d{2,4}(?:[.,]\d{2})?)\s*(?:€|EUR|Euro)?/i.exec(
      description,
    );
...
export class MessageComposer {
  ...
  async compose(listing, baseText) {
    ...
    // The system prompt explains the rules; the three worked examples that follow are the real
   ...
   *
   * Returns null when the listing offers no attribute to build the ad sentence from - a message
   * whose hook is missing (or, worse, praises a section heading) reads as broken, so no message
   * at all is the honest answer. The AI path is the primary one anyway.
   *
   * @param {Object} listing
   * @param {string} baseText
   * @returns {ComposeResult|null}
   */
