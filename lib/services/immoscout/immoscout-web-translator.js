...
function toPolyline(shape) {
  ...
  // The web UI's base64 shape can carry whitespace (line breaks, spaces) that Node's base64
  // decoder silently ignores but the strict BASE64_SHAPE_PATTERN below would reject - stripping it
  // first keeps such shapes decodable. A raw polyline never contains whitespace, so this is safe
  // for the non-base64 branch too.
  const trimmed = shape.replace(/\s+/g, '');
  if (!BASE64_SHAPE_PATTERN.test(trimmed)) return shape;

  const decoded = Buffer.from(trimmed.replace(/\.\./g, '==').replace(/\./g, '='), 'base64').toString('utf-8');
