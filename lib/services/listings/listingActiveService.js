...
export default async function runActiveChecker(opts = {}) {
  ...
  // Like its heavier sibling (price tracking) this lane is opt-in: it re-probes already-stored
  // listings, which is extra outbound traffic to the very hosts the scraper also depends on. An
  // operator who only wants new listings - and does not want to raise the ban risk - leaves it off.
  const settings = await getSettings();
  if (!settings?.activeCheckEnabled) {
    logger.debug('Active check is disabled. Skipping.');
    return;
  }
