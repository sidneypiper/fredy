  ...
  SqliteConnection.withTransaction((db) => {
    ...
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, rooms, title, image_url, description, address,
                             link, created_at, is_active, latitude, longitude, geocode_precision)
       VALUES (@id, @hash, @provider, @job_id, @price, @size, @rooms, @title, @image_url, @description, @address, @link,
               @created_at, 1, @latitude, @longitude, @geocode_precision)
    ...
    for (const item of listings) {
      const params = {
        ...
        geocode_precision: item.geocodePrecision ?? null,
      };
 ...
 * @param {'exact'|'coarse'|null} [precision] - How precise the coordinates are. `null` (the
 *   default) leaves the column untouched, which is what a not-found `-1/-1` write wants: it says
 *   nothing about precision and keeps the geocoder free to retry later.
 * @returns {void}
 */
export const updateListingGeocoordinates = (id, latitude, longitude, precision) => {
  ...
  SqliteConnection.execute(
    `UPDATE listings
     ...
     WHERE id = @id`,
    { id, latitude, longitude, precision: precision ?? null },
  );
};

/**
 * Persist the fields a detail-page fetch enriched.
 *
 * Enrichment runs after the listings were stored, so the enriched description, rooms and size
 * have to be written back to the row the pipeline already inserted - otherwise the detail page
 * would only ever reach the notification and never the database, and the UI would keep showing
 * the search-list snippet.
 *
 * @param {string} id - The listing ID.
 * @param {Object} details
 * @param {string|null} [details.description]
 * @param {number|null} [details.rooms]
 * @param {number|null} [details.size]
 * @returns {void}
 */
export const updateListingDetails = (id, { description, rooms, size }) => {
  SqliteConnection.execute(
    `UPDATE listings
     SET description = @description,
         ...
         size = @size
     WHERE id = @id`,
    { id, description: description ?? null, rooms: rooms ?? null, size: size ?? null },
  );
};

/**
 * Persist the AI-personalized message generated for a listing.
 *
 * The message is generated after the listing was stored (it needs the enriched data), so it has
 * to be written back to the row the pipeline already inserted - otherwise it would only ever
 * reach the notification and never the database, and the details view would stay empty.
 *
 * @param {string} id - The listing ID.
 * @param {string|null} message - The generated message, or null to clear it.
 * @returns {void}
 */
export const updateListingPersonalizedMessage = (id, message) => {
  SqliteConnection.execute(`UPDATE listings SET personalized_message = @message WHERE id = @id`, {
    id,
 ...
 *
 * The precision is returned alongside the coordinates so a caller reusing a coarse (postcode
 * centroid) result keeps treating it as coarse rather than silently promoting it to exact.
 *
 * @param {string} address
 * @returns {{lat: number, lng: number, precision: 'exact'|'coarse'|null}|null}
 */
export const getGeocoordinatesByAddress = (address) => {
  const row = SqliteConnection.query(
    `SELECT latitude, longitude, geocode_precision
     FROM listings
  ...
  return row ? { lat: row.latitude, lng: row.longitude, precision: row.geocode_precision ?? null } : null;
};
