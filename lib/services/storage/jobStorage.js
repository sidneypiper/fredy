    ...
    SqliteConnection.execute(
      `UPDATE jobs
       SET enabled = @enabled,
           ...
           travel_time_condition = @travelTimeCondition,
       ...
       WHERE id = @id`,
      {
        ...
        travelTimeCondition: travelTimeCondition ? toJson(travelTimeCondition) : null,
        personalizedMessage: personalizedMessage ? toJson(personalizedMessage) : null,
        dealType: dealType ?? null,
      },
    );
  } else {
    SqliteConnection.execute(
      `INSERT INTO jobs (id, user_id, enabled, name, blacklist, provider, notification_adapter, shared_with_user, spatial_filter, spec_filter, travel_time_condition, personalized_message, deal_type)
       VALUES (@id, @user_id, @enabled, @name, @blacklist, @provider, @notification_adapter, @shareWithUsers, @spatialFilter, @specFilter, @travelTimeCondition, @personalizedMessage, @dealType)`,
      {
        id,
        user_id: ownerId,
        enabled: enabled ? 1 : 0,
...
export const getJob = (jobId) => {
  const row = SqliteConnection.query(
    `SELECT j.id,
            ...
            j.travel_time_condition AS travelTimeCondition,
            j.personalized_message AS personalizedMessage,
  ...
  return {
    ...
    travelTimeCondition: fromJson(row.travelTimeCondition, null),
    personalizedMessage: fromJson(row.personalizedMessage, null),
  };
};
